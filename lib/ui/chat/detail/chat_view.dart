...
class _ChatViewState extends State<ChatView>
  ...
  Widget build(BuildContext context) {
    ...
    _chatRef.child(otherUserId!).onValue.listen((DatabaseEvent  event) { // DatabaseEvent
      final Map<dynamic, dynamic> map = event.snapshot.value as dynamic;
      if (event.snapshot.value == null) {
        ...
        itemId = map['itemId'];
        final String? _receiverId = map['receiver_id'];
      ...
      if (widget.chatFlag == PsConst.CHAT_FROM_BUYER) {
        _apiStatus = await provider.postChatImageUpload(
          ...
          await Utils.getImageFileFromAssets(
              pickedImage!, widget.psValueHolder!.chatImageSize!),
          widget.isUserOnline!,
        );
      } else {
        _apiStatus = await provider.postChatImageUpload(
